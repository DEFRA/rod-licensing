...
describe('deliverFulfilmentFiles', () => {
  ...
  it('delivers all fulfilment files marked with the Exported status to the fulfilment provider', async () => {
    ...
    readS3PartFiles.mockResolvedValue([Readable.from(['{"part": 0}']), Readable.from(['{"part": 1}'])])
    const s3DataStream = createTestableStream()
    const ftpDataStream = createTestableStream()
    createS3WriteStream.mockReturnValueOnce(s3DataStream)
    createFtpWriteStream.mockReturnValueOnce(ftpDataStream)
    const s3HashStream = createTestableStream()
    const ftpHashStream = createTestableStream()
    createS3WriteStream.mockReturnValueOnce(s3HashStream)
    createFtpWriteStream.mockReturnValueOnce(ftpHashStream)
