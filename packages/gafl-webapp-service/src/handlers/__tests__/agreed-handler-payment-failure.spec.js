...
describe('The agreed handler', () => {
  it('redirects to the payment-failed page if the GOV.UK Pay returns payment-rejected', async () => {
    ...
    expect(JSON.parse(status2)[COMPLETION_STATUS.agreed]).toBeTruthy()
    expect(JSON.parse(status2)[COMPLETION_STATUS.posted]).toBeTruthy()
    expect(JSON.parse(status2)[COMPLETION_STATUS.paymentCreated]).not.toBeTruthy()
    expect(JSON.parse(status2)[COMPLETION_STATUS.paymentCompleted]).not.toBeTruthy()
    expect(JSON.parse(status2)[COMPLETION_STATUS.finalised]).not.toBeTruthy()
  })
