...
import { AGREED, NEW_TRANSACTION, ORDER_COMPLETE, FINALISED } from '../constants.js'
...
const sessionManager = sessionCookieName => async (request, h) => {
  if (request.path.startsWith('/buy')) {
     ...
     * except for the order-complete and new transaction pages and the agreed handler itself.
     */
    if (status.agreed &&
      ![NEW_TRANSACTION.uri, AGREED.uri, ORDER_COMPLETE.uri, FINALISED.uri].includes(request.path)) {
