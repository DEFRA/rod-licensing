...
describe('The NI page', () => {
  it('returns success on requesting', async () => {
    const data = await injectWithCookies('GET', BENEFIT_NI_NUMBER.uri)
    expect(data.statusCode).toBe(200)
  })

  it('redirects back to itself on an empty response', async () => {
    const data = await injectWithCookies('POST', BENEFIT_NI_NUMBER.uri, {})
    expect(data.statusCode).toBe(302)
    expect(data.headers.location).toBe(BENEFIT_NI_NUMBER.uri)
  })

  it('redirects back to itself on an invalid response', async () => {
    const data = await injectWithCookies('POST', BENEFIT_NI_NUMBER.uri, { 'ni-number': '01234567890123456' })
    expect(data.statusCode).toBe(302)
    expect(data.headers.location).toBe(BENEFIT_NI_NUMBER.uri)
  })
