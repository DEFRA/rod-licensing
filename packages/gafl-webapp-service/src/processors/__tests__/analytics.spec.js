...
describe('tracking data transform', () => {
  it.each([
    'Salmon 12 day licence',
    'Pike 2 year licence',
    'Trout 20 second licence'
  ])('passes permit description as id', permitDescription => {
    const transaction = getSampleTransaction([
      getSamplePermission({ permitDescription })
    ])
  ...
  it.each([
    ...
    const transaction = getSampleTransaction([
      getSamplePermission({ permitSubtypeLabel, numberOfRods })
    ])
  ...
  it.each([
    'Rod Fishing Licence',
    'Fishing trawler licence',
    'Salmon tickling licence'
  ])('Generates brand from permit type label', permitTypeLabel => {
    const transaction = getSampleTransaction([
      getSamplePermission({ permitTypeLabel })
    ])
  ...
  it.each([
    ...
    const transaction = getSampleTransaction([
      getSamplePermission({ durationMagnitude, durationDesignatorLabel })
    ])
  ...
  it.each([
    ...
    'Desc 2',
    'Desc 3'
  ])('quantity is always 1', permitDescription => {
    const transaction = getSampleTransaction([
      getSamplePermission({ permitDescription })
    ])
  ...
  it.each([
    ...
    20.865,
    999.76
  ])('Generates price from permit cost', cost => {
    const transaction = getSampleTransaction([
      getSamplePermission({ cost })
    ])
  ...
  it.each([
    ...
    [['1', '2', '3', '4', '5']],
    [['eeny', 'meeny']]
  ])('generates one product for each permission', permitDescriptions => {
    const transaction = getSampleTransaction(
      permitDescriptions.map(permitDescription => getSamplePermission({ permitDescription }))
    )
