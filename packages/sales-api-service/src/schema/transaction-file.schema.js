...
export const transactionFileParamsSchema = Joi.object({
  fileName: Joi.string().min(1).trim().required().example('ExampleFile.xml')
})
...
export const createTransactionFileSchema = Joi.object({
  ...
  fileSize: Joi.string().optional().description('The size of the file'),
  totalCount: Joi.number().optional().description('The total number of sales present in the transaction file'),
  successCount: Joi.number().optional().description('The total number of sales successfully processed when importing the file'),
  errorCount: Joi.number().optional().description('The total number of sales which encountered a processing error when importing the file'),
  notes: Joi.string().optional().description('Any notes associated with the import process for the file'),
