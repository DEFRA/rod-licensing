...
export const cancelRecurringPaymentAgreement = async agreementId => {
  console.log(
    `[cancelRecurringPaymentAgreement] Calling GOV.UK Pay cancel endpoint for agreementId: ${agreementId}, URL: ${process.env.GOV_PAY_RCP_API_URL}/${agreementId}/cancel`
  )
  try {
    ...
    console.error(
      `[cancelRecurringPaymentAgreement] Error cancelling recurring payment agreement in the GOV.UK API service - agreementId: ${agreementId}`,
      err
    )
    throw err
  }
