import { isMinor, isJunior, isAdult, isSenior, MINOR_MAX_AGE, JUNIOR_MAX_AGE, RECURRING_PAYMENT_MIN_AGE, SENIOR_MIN_AGE } from '../ages.js'
...
describe('age determination', () => {
  ...
  describe('is the minimum age legally allowed a recurring card payment agreement', () => {
    it.each(Array.from({ length: RECURRING_PAYMENT_MIN_AGE - 1 }, (_v, index) => index + 1))('age of %d is not an adult', age => {
      expect(isAdult(age)).toBeFalsy()
    })
    it(`${RECURRING_PAYMENT_MIN_AGE} is an adult`, () => {
      expect(isAdult(RECURRING_PAYMENT_MIN_AGE)).toBeTruthy()
    })
    it(`${RECURRING_PAYMENT_MIN_AGE + 1} is an adult`, () => {
      expect(isAdult(RECURRING_PAYMENT_MIN_AGE + 1)).toBeTruthy()
    })
  })
})
